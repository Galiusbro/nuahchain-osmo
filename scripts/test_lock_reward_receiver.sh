...
nuahd add-genesis-account $(nuahd keys show validator -a --keyring-backend=test) 100000000000stake,100000000000uosmo,100000000000uion --home=$HOME/.nuahd
...
update_genesis () {    
    cat $HOME/.nuahd/config/genesis.json | jq "$1" > $HOME/.nuahd/config/tmp_genesis.json && mv $HOME/.nuahd/config/tmp_genesis.json $HOME/.nuahd/config/genesis.json
}
...
tmux new -s validator -d nuahd start --home=$HOME/.nuahd

sleep 7
# create pool
nuahd tx gamm create-pool --pool-file=./stake-uosmo.json --from=validator --keyring-backend=test --chain-id=testing --yes --fees=1000000stake
sleep 7

# do a swap in the pool created
nuahd tx gamm swap-exact-amount-in 100000uosmo 50000 --swap-route-pool-ids=1 --swap-route-denoms=stake --from=validator --keyring-backend=test --chain-id=testing --yes --fees=10000stake
sleep 7

# create a lock up with lock duration 360h
nuahd tx lockup lock-tokens 10000000000000000000gamm/pool/1 --duration=360h --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# register superfluid asset through governance
nuahd tx gov submit-proposal set-superfluid-assets-proposal --title="set superfluid assets" --description="set superfluid assets description" --superfluid-assets="gamm/pool/1" --deposit=10000000stake --from=validator --chain-id=testing --keyring-backend=test --broadcast-mode=block --yes --fees=10000stake
sleep 7
nuahd tx gov deposit 1 10000000stake --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake 
sleep 7
nuahd tx gov vote 1 yes --from=validator --keyring-backend=test --chain-id=testing --yes --fees=100000stake
sleep 7

# Command to extract operator_address
operator_address=$(./nuahd q staking validators | yq e '.validators[].operator_address' -)

# Use the operator_address in the nuahd command
nuahd tx superfluid delegate 1 ${operator_address} --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# add different account that would be set as lock reward receiver
nuahd keys add test1 --keyring-backend=test 
sleep 7

# set lock reward receiver 
nuahd tx lockup set-reward-receiver-address 1 $(./nuahd keys show test1 -a --keyring-backend=test) --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# query lock reward receiver 
# nuahd q lockup lock-reward-receiver 1

# the new address would be receiving the lock rewards, while the original account does not receive the rewards anymore
# nuahd q bank balances $(nuahd keys show test1 -a --keyring-backend=test)
# nuahd q bank balances $(nuahd keys show validator -a --keyring-backend=test)

# set the lock reward receiver back to the original address
nuahd tx lockup set-reward-receiver-address 1 $(./nuahd keys show validator -a --keyring-backend=test) --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7
...
nuahd tx lockup lock-tokens 10000000000000000000gamm/pool/1 --duration=1s --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# now let's also try adding lock incentives as well
nuahd tx incentives create-gauge gamm/pool/1 1000000uion 0 --duration 120s  --start-time 1688115600 --epochs 100 --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7
