...
nuahd keys add demo --keyring-backend test
VAL=$(nuahd keys show -a validator --keyring-backend test)
DEMO=$(nuahd keys show -a demo --keyring-backend test)
...
nuahd tx wasm instantiate $CODE "$INIT" --label "First Coin" --no-admin --from validator \
...
nuahd query wasm contract-state smart $CONTRACT "$QUERY"
echo "Demo Balance:"
nuahd query wasm contract-state smart $CONTRACT "$QUERY_DEMO"
...
nuahd tx wasm execute $CONTRACT "$TRANSFER" --from validator --keyring-backend test \
