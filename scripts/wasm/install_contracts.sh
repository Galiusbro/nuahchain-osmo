...
nuahd tx gov submit-proposal wasm-store $CONTRACT.wasm --title "Add $CONTRACT" \
...
nuahd query gov proposal $PROPOSAL

nuahd tx gov deposit $PROPOSAL 10000000stake --from validator --keyring-backend test \
    --chain-id $CHAIN_ID -y -b block --gas 5000000 --gas-prices 0.025stake

nuahd tx gov vote $PROPOSAL yes --from validator --keyring-backend test \
