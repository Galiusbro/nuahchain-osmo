# Mainnet Upgrade Guide: From Version v22 to v23

## Overview

- **v23 Proposal**: [Proposal Page](https://www.mintscan.io/osmosis/proposals/730)
- **v23 Upgrade Block Height**: 13899375
- **v23 Upgrade Countdown**: [Block Countdown](https://www.mintscan.io/osmosis/blocks/13899375)
...
mkdir -p ~/.nuahd/cosmovisor/upgrades/v22/bin
cp $GOPATH/bin/nuahd ~/.nuahd/cosmovisor/upgrades/v22/bin
```
...
```sh
mkdir -p ~/.nuahd/cosmovisor/upgrades/v23/bin
cd $HOME/osmosis
git pull
git checkout v23.0.0
make build
cp build/nuahd ~/.nuahd/cosmovisor/upgrades/v23/bin
```

At the designated block height, Cosmovisor will automatically upgrade to version v23.
...
1. Monitor Osmosis until it reaches the specified upgrade block height: 13899375.
