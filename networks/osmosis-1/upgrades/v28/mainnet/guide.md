...
mkdir -p ~/.nuahd/cosmovisor/upgrades/v27/bin
cp $GOPATH/bin/nuahd ~/.nuahd/cosmovisor/upgrades/v27/bin
```
...
```sh
mkdir -p ~/.nuahd/cosmovisor/upgrades/v28/bin
cd $HOME/osmosis
git pull
git checkout v28.0.0
make build
cp build/nuahd ~/.nuahd/cosmovisor/upgrades/v28/bin
```
