# Mainnet Upgrade Guide: From Version v19 to v20
...
mkdir -p ~/.nuahd/cosmovisor/upgrades/v19/bin
cp $GOPATH/bin/nuahd ~/.nuahd/cosmovisor/upgrades/v19/bin
```
...
```sh
mkdir -p ~/.nuahd/cosmovisor/upgrades/v20/bin
cd $HOME/osmosis
git pull
git checkout v20.2.1
make build
cp build/nuahd ~/.nuahd/cosmovisor/upgrades/v20/bin
```

At the designated block height, Cosmovisor will automatically upgrade to version v20.
