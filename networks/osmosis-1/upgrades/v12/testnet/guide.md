...
Copy the current v11 nuahd binary into the
cosmovisor/genesis folder and v11 folder.

```{.sh}
cp $GOPATH/bin/nuahd ~/.nuahd/cosmovisor/genesis/bin
mkdir -p ~/.nuahd/cosmovisor/upgrades/v11/bin
cp $GOPATH/bin/nuahd ~/.nuahd/cosmovisor/upgrades/v11/bin
```
...
make build
cp build/nuahd ~/.nuahd/cosmovisor/upgrades/v12/bin
```
...
1. Wait for Osmosis to reach the upgrade height (6799850)
