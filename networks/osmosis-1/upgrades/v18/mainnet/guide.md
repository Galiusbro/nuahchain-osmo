...
Copy the current v17 nuahd binary into the
...
cp $GOPATH/bin/nuahd ~/.nuahd/cosmovisor/upgrades/v17/bin
```
...
```{.sh}
mkdir -p ~/.nuahd/cosmovisor/upgrades/v18/bin
cd $HOME/osmosis
git pull
git checkout v18.0.0
make build
cp build/nuahd ~/.nuahd/cosmovisor/upgrades/v18/bin
```
