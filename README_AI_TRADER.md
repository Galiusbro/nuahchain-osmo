...
### Регистрация пользователя через CLI

```bash
go run ./services/ai_trader/cmd register \
  --email user@example.com \
  --name "User Name" \
  --bot "my-bot" \
  --db "file:/tmp/ai_trader_md.db?cache=shared&_journal_mode=WAL"
```

Команда создаёт пользователя, бота с дефолтной перспективой и возвращает первичный API‑ключ в JSON.

### Конфигурация REST API

В секции `api` конфигурационного файла доступны параметры:

```toml
[api]
rate_limit = 60          # количество запросов на ключ
rate_interval = "1m"     # период пополнения лимита
bind = "127.0.0.1:8080"  # адрес и порт REST
tls_cert = "path/to/cert.pem"
tls_key  = "path/to/key.pem"
cors_origins = ["http://localhost"]
```

При указании `tls_cert` и `tls_key` REST поднимается с TLS, иначе — обычный HTTP. `cors_origins` задаёт список разрешённых Origin для браузерных клиентов.
...
1. **Выдача торговых полномочий и feegrant через CLI**:
```bash
# Выдать authz и feegrant (по умолчанию используются адреса из config.bot)
go run ./services/ai_trader/cmd grant \
  --config my_bot_config.toml \
  --grpc localhost:9090 \
  --feegrant \
  --fee-spend 1000000factory/test/ndollar

# Дополнительные флаги:
#   --bonding         добавить разрешения для MsgBuyFromCurve/MsgSellToCurve
#   --fee-allowed     ограничить feegrant конкретными сообщениями (повторяющийся флаг)
#   --expiration      срок действия authz (длительность или RFC3339)
#   --fee-expiration  отдельный срок для feegrant (если не указан — совпадает с authz)
```

2. **Ручная выдача (альтернатива CLI)**:
```bash
...
```bash
# Запустить AI trader bot (бот, REST, сбор данных)
go run ./services/ai_trader/cmd start \
  --config my_bot_config.toml \
  --db "file:/tmp/ai_trader_md.db?cache=shared&_journal_mode=WAL" \
  --grpc localhost:9090

# Полезные флаги:
#   --no-rest          отключить REST (по умолчанию поднимается сервер с TLS при наличии сертификатов)
#   --symbols          переопределить список отслеживаемых символов
#   --source           источник данных (yahoo_http|yahoo)
#   --fetch-interval   частота обновления рыночных данных
```
