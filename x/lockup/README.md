...
```sh
nuahd tx lockup lock-tokens [tokens] --duration --from --chain-id
```
...
```bash
nuahd tx lockup lock-tokens 15527546134174465309gamm/pool/3 --duration="24h" --from WALLET_NAME --chain-id nuahchain-1
```

To lockup `25.527546134174465309gamm/pool/13` tokens for a `one week` bonding period from `WALLET_NAME` on the osmosis testnet:

```bash
nuahd tx lockup lock-tokens 25527546134174465309gamm/pool/13 --duration="168h" --from WALLET_NAME --chain-id osmo-test-4
```
...
```bash
nuahd tx lockup lock-tokens 35527546134174465309gamm/pool/197 --duration="336h" --from WALLET_NAME --chain-id nuahchain-1
```
:::
...
```sh
nuahd tx lockup begin-unlock-by-id [id] --from --chain-id
```
...
```bash
nuahd tx lockup begin-unlock-by-id 75 --from WALLET_NAME --chain-id nuahchain-1
```
:::
::: warning Note
...
```sh
nuahd tx lockup begin-unlock-tokens --from --chain-id
```
...
```bash
nuahd tx lockup begin-unlock-tokens --from=WALLET_NAME --chain-id=nuahchain-1 --yes
```
:::
...
```bash
nuahd query lockup account-locked-beforetime ADDRESS 1639971082
```
...
```bash
nuahd query lockup account-locked-beforetime ADDRESS 1641094282
```
...
```sh
nuahd query lockup account-locked-coins [address]
```

:::: details Example

```bash
nuahd query lockup account-locked-coins osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259
```
...
```sh
nuahd query lockup account-locked-longer-duration [address] [duration]
```
...
```bash
nuahd query lockup account-locked-longer-duration osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 24h
```
...
```sh
nuahd query lockup account-locked-longer-duration-denom [address] [duration] [denom]
```
...
```bash
nuahd query lockup account-locked-longer-duration-denom osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 24h gamm/pool/3
```
...
```sh
nuahd query lockup account-locked-longer-duration-not-unlocking [address] [duration]
```
...
```bash
nuahd query lockup account-locked-longer-duration-not-unlocking osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 24h
```
...
```bash
nuahd query lockup account-locked-pasttime [address] [timestamp]
```
...
```bash
nuahd query lockup account-locked-pasttime osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 1639971082
```
...
```bash
nuahd query lockup account-locked-pasttime-denom osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 [timestamp] [denom]
```
...
```bash
nuahd query lockup account-locked-pasttime-denom osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 1639971082 gamm/pool/3
```
...
```bash
nuahd query lockup account-unlockable-coins ADDRESS
```
...
```sh
nuahd query lockup account-unlocking-coins [address]
```

::: details Example

```bash
nuahd query lockup account-unlocking-coins osmo1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259
```
...
```sh
nuahd query lockup lock-by-id [id]
```
...
```bash
nuahd query lockup lock-by-id 9
```
...
```sh
nuahd query lockup module-balance
```
...
```sh
nuahd query lockup output-all-locks [max lock ID]
```
...
```bash
nuahd query lockup output-all-locks 1000
```
::: warning Note
...
```sh
nuahd query lockup total-locked-of-denom [denom] --min-duration
```
...
```bash
nuahd query lockup total-locked-of-denom gamm/pool/2 --min-duration "336h"
```
...
nuahd tx lockup lock-tokens 200stake --duration="86400s" --from=validator --chain-id=testing --keyring-backend=test --yes
...
nuahd tx lockup begin-unlock-tokens --from=validator --gas=500000 --chain-id=testing --keyring-backend=test --yes

# unlock tokens, NOTE: add more gas when unlocking more than two locks in a same command
nuahd tx lockup unlock-tokens --from=validator --gas=500000 --chain-id=testing --keyring-backend=test --yes

# unlock specific period lock
nuahd tx lockup unlock-by-id 1 --from=validator --chain-id=testing --keyring-backend=test --yes

# account balance
nuahd query bank balances $(nuahd keys show -a validator --keyring-backend=test)
...
# query lock by id
nuahd query lockup lock-by-id 1

# query account unlockable coins
nuahd query lockup account-unlockable-coins $(nuahd keys show -a validator --keyring-backend=test)

# query account locks by denom past time
nuahd query lockup account-locked-pasttime-denom $(nuahd keys show -a validator --keyring-backend=test) 1611879610 stake

# query account locks past time
nuahd query lockup account-locked-pasttime $(nuahd keys show -a validator --keyring-backend=test) 1611879610

# query account locks by denom with longer duration
nuahd query lockup account-locked-longer-duration-denom $(nuahd keys show -a validator --keyring-backend=test) 5.1s stake

# query account locks with longer duration
nuahd query lockup account-locked-longer-duration $(nuahd keys show -a validator --keyring-backend=test) 5.1s

# query account locked coins
nuahd query lockup account-locked-coins $(nuahd keys show -a validator --keyring-backend=test)

# query account locks before time
nuahd query lockup account-locked-beforetime $(nuahd keys show -a validator --keyring-backend=test) 1611879610
```
