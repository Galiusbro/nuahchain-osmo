...
```shell
nuahd tx gov submit-proposal update-pool-incentives [gaugeIds] [weights]
```
...
```shell
nuahd tx gov submit-proposal update-pool-incentives 2,3 100,200
```
...
```sh
nuahd tx poolincentives replace-pool-incentives [gaugeIds] [weights] [flags]
```
...
```bash
nuahd tx poolincentives replace-pool-incentives proposal.json --from --chain-id
```
...
```sh
nuahd tx poolincentives update-pool-incentives [gaugeIds] [weights] [flags] --from --chain-id
```
...
```bash
nuahd tx gov submit-proposal update-pool-incentives proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
```sh
nuahd query poolincentives distr-info
```
...
```sh
nuahd query pool-incentives external-incentivized-gauges
```
...
```sh
nuahd query poolincentives gauge-ids [pool-id] [flags]
```
...
```bash
nuahd query poolincentives gauge-ids 1
```
...
```sh
nuahd query poolincentives incentivized-pools [flags]
```

::: details Example

```bash
nuahd query poolincentives incentivized-pools
```
...
```sh
nuahd query poolincentives lockable-durations [flags]
```

::: details Example

```bash
nuahd query poolincentives lockable-durations
```
...
```sh
nuahd query poolincentives params [flags]
```

::: details Example

```bash
nuahd query poolincentives params
```
