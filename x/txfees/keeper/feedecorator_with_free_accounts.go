...
// FreeAccountChecker defines an interface for checking if an account is fee-exempt
type FreeAccountChecker interface {
	IsFreeAccount(ctx sdk.Context, addr sdk.AccAddress) bool
}

// MempoolFeeDecoratorWithFreeAccounts will check if the transaction's fee is at least as large
// as the local validator's minimum gasFee (defined in validator config).
// If fee is too low, decorator returns error and tx is rejected from mempool.
// Note this only applies when ctx.CheckTx = true
// If fee is high enough or not CheckTx, then call next AnteHandler
// CONTRACT: Tx must implement FeeTx to use MempoolFeeDecorator.
type MempoolFeeDecoratorWithFreeAccounts struct {
	TxFeesKeeper       Keeper
	Opts               types.MempoolFeeOptions
	FreeAccountChecker FreeAccountChecker
}

// NewMempoolFeeDecoratorWithFreeAccounts creates a new MempoolFeeDecorator with free account checking
func NewMempoolFeeDecoratorWithFreeAccounts(txFeesKeeper Keeper, opts types.MempoolFeeOptions, freeAccountChecker FreeAccountChecker) MempoolFeeDecoratorWithFreeAccounts {
	...
	return MempoolFeeDecoratorWithFreeAccounts{
...
func (mfd MempoolFeeDecoratorWithFreeAccounts) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	// Check if any signer is a free account
	if mfd.FreeAccountChecker != nil {
		sigTx, ok := tx.(authsigning.SigVerifiableTx)
		if ok {
			signers, err := sigTx.GetSigners()
			if err == nil {
				for _, signer := range signers {
					if mfd.FreeAccountChecker.IsFreeAccount(ctx, signer) {
						// Skip fee checking for free accounts
...
func (mfd MempoolFeeDecoratorWithFreeAccounts) getMinBaseGasPrice(ctx sdk.Context, baseDenom string, simulate bool, feeTx sdk.FeeTx) osmomath.Dec {
...
func (mfd MempoolFeeDecoratorWithFreeAccounts) GetMinBaseGasPriceForTx(ctx sdk.Context, baseDenom string, tx sdk.FeeTx) osmomath.Dec {
	// Use the original decorator's logic for now
	originalDecorator := NewMempoolFeeDecorator(mfd.TxFeesKeeper, mfd.Opts)
	return originalDecorator.GetMinBaseGasPriceForTx(ctx, baseDenom, tx)
}