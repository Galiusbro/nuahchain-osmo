...
Localrelayer is a local testing environment composed of two localNuah instances connected by a relayer.
...
|------------------|------------|--------------------------|
| `localnuah-a` | `RPC`      | <http://localhost:26657> |
| `localnuah-a` | `REST/LCD` | <http://localhost:1317>  |
| `localnuah-a` | `gRPC`     | <http://localhost:9090>  |
| `localnuah-a` | `faucet`   | <http://localhost:8080>  |
| `localnuah-b` | `RPC`      | <http://localhost:36657> |
| `localnuah-b` | `REST/LCD` | <http://localhost:31317> |
| `localnuah-b` | `gRPC`     | <http://localhost:39090> |
| `localnuah-b` | `faucet`   | <http://localhost:38080> |
...
|------------------|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `localnuah-a` | `validator-a` | *family album bird seek tilt color pill danger message abuse manual tent almost ridge boost blast high comic core quantum spoon coconut oyster remove*            |
| `localnuah-a` | `faucet`      | *increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great*     |
| `localnuah-a` | `relayer`     | *black frequent sponsor nice claim rally hunt suit parent size stumble expire forest avocado mistake agree trend witness lounge shiver image smoke stool chicken* |
| `localnuah-b` | `validator-b` | *family album bird seek tilt color pill danger message abuse manual tent almost ridge boost blast high comic core quantum spoon coconut oyster remove*            |
| `localnuah-b` | `faucet`      | *increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great*     |
| `localnuah-b` | `relayer`     | *black frequent sponsor nice claim rally hunt suit parent size stumble expire forest avocado mistake agree trend witness lounge shiver image smoke stool chicken* |
...
```bash
 ⠿ Container localrelayer-localnuah-b-1  Created
 ⠿ Container localrelayer-localnuah-a-1  Created
...
d90ec29c7a6f   local:osmosis                  "/osmosis/setup.sh"      About a minute ago   Up 3 seconds   26656/tcp, 0.0.0.0:31317->1317/tcp, 0.0.0.0:39090->9090/tcp, 0.0.0.0:36657->26657/tcp   localrelayer-localnuah-b-1
e36cead49a07   local:osmosis                  "/osmosis/setup.sh"      About a minute ago   Up 3 seconds   0.0.0.0:1317->1317/tcp, 0.0.0.0:9090->9090/tcp, 0.0.0.0:26657->26657/tcp, 26656/tcp     localrelayer-localnuah-a-1
```
...
Check `localnuah-a` status:

```bash
curl -s http://localhost:26657/status
```

Check `localnuah-b` status:
...
```bash
❯ nuahd keys add my-account --keyring-backend test
...
# Use the following endpoint for localnuah-b:
...
hermes tx ft-transfer --timeout-seconds 1000 \
    --dst-chain localnuah-a \
    --src-chain localnuah-b \
    --src-port transfer \
