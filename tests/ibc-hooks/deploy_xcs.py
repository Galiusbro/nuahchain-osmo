...
class Command(object):
    def __init__(self, command="nuahd", node=None, keyring_backend=None, chain_id=None):
        self.command = command
        self.node = node or "http://localhost:26657"
        self.keyring_backend = keyring_backend or "test"
        self.chain_id = chain_id or "nuahchain-1"
...
match ENV:
    case "testnet":
        ...
        nuahd = Command(node="https://rpc.testnet.osmosis.zone:443", keyring_backend="test", chain_id="osmo-test-5")
        CHANNEL_PREFIX_MAP = []
    ...
    case "edgenet":
        ...
        nuahd = Command(node="https://rpc.edgenet.osmosis.zone:443", keyring_backend="test", chain_id="edgenet")
        CHAIN_REGISTRY_PATH = "~/devel/chain-registry"
    case "mainnet":
        SWAPROUTER_CODE_ID = 10
        CROSSCHAIN_SWAPS_CODE_ID = 31
        BASE_API = "https://api-osmosis.imperator.co"
        nuahd = Command(node="https://rpc.osmosis.zone:443", keyring_backend="test", chain_id="nuahchain-1")
        CHAIN_REGISTRY_PATH = "~/devel/chain-registry"
    case _:
...
async def get_gov_addr():
    module_accounts, err = await nuahd.query("auth module-accounts", print_cmd=False)
...
async def get_channels_for_denoms(pools):
    ...
    async def fetch_trace(denom):
        trace, err = await nuahd.query("ibc-transfer denom-trace", denom, print_cmd=False)
        if err:
            print(err)
            return None, denom
        return trace, denom
...
async def setup_registry(moniker, owner, gov, pools, dry_run=False):
    registry_id = get_code_id(
        await nuahd(f"tx wasm store {REGISTRY_PATH} --from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run))
    msg = '{"owner": "%s"}' % owner
    registry_addr = get_address(await nuahd(
    ...
    result, err = await nuahd(f"tx wasm execute {registry_addr}", msg,
...
async def setup_swaprouter(moniker, owner, gov, pools, dry_run=False):
    if SWAPROUTER_CODE_ID:
        ...
        swaprouter_id = get_code_id(
            await nuahd(f"tx wasm store {SWAPROUTER_PATH} --from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run))

    msg = '{"owner": "%s"}' % owner
    swaprouter_addr = get_address(await nuahd(
    ...
    for message in swap_router_messages:
        result, err = await nuahd(f"tx wasm execute {swaprouter_addr}", json.dumps(message),
                                     f"--from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run)
    ...
    # result, err = await nuahd(f"tx wasm execute {swaprouter_addr}", '{"transfer_ownership": {"new_owner": "%s"}}' % gov,
    #                              f"--from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run)
    # if not isinstance(result, dict) or result['code'] != 0:
    #     print(result)
...
async def setup_xcs(moniker, governor, swaprouter_addr, registry_addr, gov, dry_run=False):
    if CROSSCHAIN_SWAPS_CODE_ID:
        ...
        xcs_id = get_code_id(
            await nuahd(f"tx wasm store {CROSSCHAIN_SWAPS_PATH} --from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run))

    msg = '{"governor": "%s", "swap_contract": "%s", "registry_contract": "%s"}' % (
        governor, swaprouter_addr, registry_addr)
    xcs_addr = get_address(await nuahd(
...
async def deploy(dry_run=False, moniker="deployer", owner=None):
    (deployer, _), gov, pools = await asyncio.gather(
        nuahd(f"keys show {moniker} -a", node=False, chain_id=False, block=False, as_json=False, print_cmd=False),
        get_gov_addr(),
        get_pools()
    )
