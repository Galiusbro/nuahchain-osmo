...
func (suite *HooksTestSuite) forceContractToEmitAckForPacket(NUAHApp *app.NUAHApp, ctx sdk.Context, contractAddr sdk.AccAddress, packet channeltypes.Packet, success bool) ([]byte, error) {
...
func (suite *HooksTestSuite) TestWasmHooksAsyncAcks() {
	...
	allAcks := NUAHApp.IBCKeeper.ChannelKeeper.GetAllPacketAcks(suite.chainA.GetContext())
	...
	_, err = suite.forceContractToEmitAckForPacket(NUAHApp, suite.chainA.GetContext(), contractAddr, alreadyAckedPacket, true)
	...
	NUAHApp.IBCHooksKeeper.SetParams(suite.chainA.GetContext(), params)
	...
	for _, tc := range testCases {
		...
		_, err = suite.forceContractToEmitAckForPacket(NUAHApp, suite.chainA.GetContext(), contractAddr2, packet, tc.success)
		...
		_, err = suite.forceContractToEmitAckForPacket(NUAHApp, ctx, contractAddr, packet, tc.success)
		totalExpectedAcks++
		suite.Require().NoError(err)
